...
if __name__ == '__main__':
    ...
    bevformer_results = mmcv.load('json_val/RERF.json')
    sample_token_list = list(bevformer_results['results'].keys())
    sample_token_list.sort()
    for id in range(50, 100):
        render_sample_data(sample_token_list[id], pred_data=bevformer_results, out_path='result_vis/gt/' + sample_token_list[id])