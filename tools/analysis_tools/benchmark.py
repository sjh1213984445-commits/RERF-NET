...
def parse_args():
    ...
    parser.add_argument('config', help='test config file path', default='projects/RERF/configs/rerf_lidar-cam.py')
    parser.add_argument('checkpoint', help='checkpoint file', default='pretrain_model/val-best.pth')
...
def main():
    ...
    # build dataloader
    # cfg.test_dataloader.dataset.pipeline[2].sweeps_num = 1
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader):
            ...
            # with open("voxel_time.json","r") as f:
            #     voxel_time = json.load(f)
            # elapsed = elapsed - voxel_time
            if i >= num_warmup:
