...
def density(pointcloud, severity, gt_boxes):
...
def cutout(pointcloud, severity, gt_boxes):
    N, C = pointcloud.shape
...
def gaussian(pointcloud, severity, gt_boxes):
...
def uniform(pointcloud, severity, gt_boxes):
    N, C = pointcloud.shape
    c = [0.02, 0.04, 0.06, 0.08, 0.10][severity - 1]
...
def impulse(pointcloud, severity, gt_boxes):
    N, C = pointcloud.shape
...
def to_Max2(points, gt_boxes_lidar):
    ...
    points = np.concatenate([points_rot, points[:, 3:]], axis=-1)
...
def shear(pointcloud, severity,gt_boxes):
    ...
    pts_obj_max2_crp = np.concatenate([new_pc, pts_obj_max2[:, 3:]], axis=-1)
...
def rotation(pointcloud,severity,gt_boxes):
    ...
    pts_obj_max2_crp = np.concatenate([pts_rotated, pts_obj_max2[:,3:]],axis=-1)
    # convert to lidar
    pts_cor = to_Lidar(pts_obj_max2_crp, gt_boxes)

    return pts_cor

def moving_object(pointcloud, severity, gt_boxes):
...
def pick_bbox(cor,slevel,data,pointcloud):
    ...
    bboxes = data[0].numpy()
    flag1 = 0
    point_indices = box_np_ops.points_in_rbbox(xyz, bboxes)
    xyz_out = []
    xyz_out.append(xyz[point_indices.sum(axis=-1)==0]) # add background
    ...
    for i in range(len(bboxes)):
        ...
        if len(pcd_2) != 0:
            ...
            xyz_out.append(pcd_2)
    xyz_out = np.concatenate(xyz_out, axis=0)
    return xyz_out
