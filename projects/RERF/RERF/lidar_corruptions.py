...
def rain_sim(pointcloud,severity):
    ...
    rain_sim = lisa.LISA(show_progressbar=False)
...
def snow_sim_nus(pointcloud,severity):
    ...
    then: change the utils_cor in wet ground
    '''
    # import sys
...
def scene_glare_noise(pointcloud, severity):
    ...
    pointcloud[index][:,:4] += np.random.normal(size=(c, 4)) * 2.0
    return pointcloud
...
def lidar_crosstalk_noise(pointcloud, severity):
    ...
    index = np.random.choice(N, c, replace=False)
    pointcloud[index][:,:4] += np.random.normal(size=(c, 4)) * 3.0
    return pointcloud
