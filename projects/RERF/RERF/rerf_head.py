...
from .lightattention import lightMMatten
...
class CoreNetHead(nn.Module):
    ...
    def __init__(
        ...
        test_cfg=None,
        bbox_coder=None,
        img_fuse: Optional[dict] = None,
        pts_fuse: Optional[dict] = None,
        aux_pre: Optional[dict] = None,
        aux_loss: Optional[dict] = None,
    ):
        super(CoreNetHead, self).__init__()
        ...
        self.decoder = nn.ModuleList() #CA in LRF
    ...
    def forward_DBP(self, inputs, metas):
        fused_feat = inputs[0]
        img_feat = inputs[1]
        pts_feat = inputs[2]
        ...
        top_proposals = heatmap.view(batch_size, -1).contiguous().argsort(
            dim=-1, descending=True)[..., :self.num_proposals]
        top_proposals_class = top_proposals // heatmap.shape[-1]
        top_proposals_index = top_proposals % heatmap.shape[-1]

        # use pts_feat as bbox query_feat, use fuse_feat as cls query feat
        ...
        one_hot = F.one_hot(
            top_proposals_class,
            num_classes=self.num_classes).permute(0, 2, 1).contiguous()
        ...
        for i in range(self.num_decoder_layers):
            ...
            query_pos = res_layer['center'].detach().clone().permute(0, 2, 1).contiguous()
        ...
        # final_res.update(aux_g_res_layer)
        return [final_res]
    ...
    def forward_single(self, inputs, metas):
        ...
        if len(inputs)==1:
            ...
            res_dbp = self.forward_DBP(inputs, metas)
            return res_dbp
    ...
    def loss_by_feat(self, preds_dicts: Tuple[List[dict]],
        ...
        if self.aux_pre == False or self.aux_loss == False:
            return loss_dict
        # 计算aux的 loss
        ...
        (
            ...
            aux_matched_ious,
            aux_heatmap,
        ) = self.get_targets(batch_gt_instances_3d, [aux_preds])
