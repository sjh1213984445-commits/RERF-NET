...
class BaseDepthTransform(BaseViewTransform):
    ...
    def forward(
        ...
        **kwargs,
        ...
        '''
        mats_dict = dict()
        mats_dict['intrin_mats'] = cam_intrinsic.unsqueeze(1)
        mats_dict['ida_mats'] = img_aug_matrix.unsqueeze(1)
        mats_dict['sensor2ego_mats'] = camera2lidar.unsqueeze(1)[..., :3, :4]
        mats_dict['bda_mat'] = lidar_aug_matrix
        ...
        x_ray, pred_depth = self.get_cam_feats(img, mats_dict)
        
        x_ray = self.bev_pool(geom_ray, x_ray)
        ...
        x_point = self.bev_pool(cam_points, img.view(B,N,C,H,W).unsqueeze(-1).permute(0,1,5,3,4,2))

        x_fusion = self.view_fusion(torch.cat([x_ray, x_point], dim=1))
       
        x_fusion = self.downsample(x_fusion)
        return x_fusion, depth, pred_depth

@MODELS.register_module()
class DualTransform(BaseDepthTransform):
    ...
    def __init__(
        ...
        self.depthnet = DepthNet(in_channels, in_channels*2, out_channels, int((self.dbound[1] - self.dbound[0]) / self.dbound[2]))
        if use_drf:
            self.depth_aggregation_net = DepthAggregation(out_channels, out_channels, out_channels)
        else: 
            self.depth_aggregation_net = None
        ...
        self.view_fusion = nn.Sequential(
            nn.Conv2d(80+64, 256, 3, padding=1, bias=False),
            nn.BatchNorm2d(256),
            nn.ReLU(True),
        )
        if downsample > 1:
            ...
            self.downsample = nn.Sequential(
                nn.Conv2d(
                    256, final_channels, 3, padding=1, bias=False),
                ...
                nn.ReLU(True),
                nn.Conv2d(
                    final_channels, final_channels, 3, padding=1, bias=False),
                nn.BatchNorm2d(final_channels),
                nn.ReLU(True),
            )
        else:
    ...
    def get_cam_feats(self, x, mats_dict):
        B, N, C, fH, fW = x.shape
        x = x.view(B*N,C,fH,fW)
        depth_feature = self.depthnet(x, mats_dict)
        
        depth = depth_feature[:, :self.D].softmax(dim=1, dtype=depth_feature.dtype)
        ############################
        
        x = depth.unsqueeze(1) * depth_feature[:, self.D:(self.D + self.C)].unsqueeze(2)

        if self.depth_aggregation_net:
            x = self._forward_voxel_net(x) # depth refinement module in BEVdepth
    ...
    def forward(self, *args, **kwargs):
        x, depth, pred_depth = super().forward(*args, **kwargs)
        return x, depth, pred_depth
