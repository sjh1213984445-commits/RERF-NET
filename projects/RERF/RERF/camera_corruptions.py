import sys
# sys.path.append('utils_cor')


# import imgaug.augmenters as iaa
# import .utils_cor.imgaug.augmenters as iaa
    ...
    def __call__(self, image, lidar2img, bboxes_centers, bboxes_corners, watch_img=False, file_path='') -> np.array:
        ...
        for idx_b in range(bboxes_num):
            ...
            if corners_uv.shape[0] == 0:
                continue
            hull = cv2.convexHull(corners_uv.numpy().astype(np.int32))
...
class ImageBBoxMotionBlurLeftRight(): 
    ...
    def __call__(self, image, lidar2img, bboxes_centers, bboxes_corners, watch_img=False, file_path='') -> np.array:
        ...
        self.iaa_seq = iaa.Sequential([
            iaa.blur.MotionBlur(k=kernel_size, angle=90),
        ])
        # bboxes_corners 3*8*3   tensor 
        # lidar2img 4*4   np.array
        canvas = image.copy()
...
class ImageAddGaussianNoise():
    def __init__(self, severity, seed) -> None:
        self.iaa_seq = iaa.Sequential([
            iaa.GaussianNoise(severity=severity, seed=seed),
        ])
...
class ImageAddImpulseNoise():
    def __init__(self, severity, seed) -> None:
        self.iaa_seq = iaa.Sequential([
            iaa.ImpulseNoise(severity=severity, seed=seed),
        ])
...
class ImageAddSnow():
    def __init__(self, severity, seed) -> None:
        self.iaa_seq = iaa.Sequential([
            iaa.Snow(severity=severity, seed=seed),
        ])
...
class ImageAddFog():
    def __init__(self, severity, seed) -> None:
        self.iaa_seq = iaa.Sequential([
            iaa.Fog(severity=severity, seed=seed),
        ])
        self.gray_ratio = [
            0.1,
            0.2,
