    ...
    def __init__(
        ...
        if downsample > 1:
            ...
            self.downsample = nn.Sequential(
                nn.Conv2d(
                    out_channels, final_channels, 3, padding=1, bias=False),
...
class BaseDepthTransform(BaseViewTransform):
    ...
    def forward(
        ...
        **kwargs,
        ...
        x, pred_depth = self.get_cam_feats(img, depth, mats_dict)
...
class DepthLSSTransform(BaseDepthTransform):
    ...
    def __init__(
        ...
        self.depthnet = DepthNet(256+64, 512, 80, int((self.dbound[1] - self.dbound[0]) / self.dbound[2]))
        self.depth_aggregation_net = DepthAggregation(80, 80, 80)
        if downsample > 1:
            assert downsample == 2, downsample
            self.downsample = nn.Sequential(
                nn.Conv2d(
    ...
    def get_cam_feats(self, x, gt_depth, mats_dict):
        B, N, C, fH, fW = x.shape
        
        d = gt_depth.view(B * N, *gt_depth.shape[2:])
        x = x.view(B * N, C, fH, fW)
        d = self.dtransform(d)
